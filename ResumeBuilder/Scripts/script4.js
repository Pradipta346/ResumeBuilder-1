...
function generateExp(experience) {
    ...
    experience.forEach(function (item) {
        if (item.PositionTitle) {
            ...
            hasContent = true;
            var jobHtml = `
                    <div style="margin-bottom: 0;"><b>${item.PositionTitle} | ${formattedstartdate} - ${formattedenddate}</b></div>
                    <h5 style="margin-top: 5px; margin-bottom: 5px; font-size: 14px;"><b><i>${item.EmployerName}, ${item.Location}</i></b></h5>
                    <ul style="margin-top: 5px;"><li style="font-size: 14px;">${item.Description}</li></ul>
                    ${item.RoleResp ? `<ul style="margin-top: 5px;"><li  style="font-size: 14px;">${item.RoleResp}</li></ul>` : ''}
            `;
...
function generateEdu(education) {
    ...
    education.forEach(function (item) {
        if (item.SchoolName) {
            ...
            hasContent = true;
            var eduHtml = `
                    <div><b>${item.EducationDegree}, ${item.Major}</b></div>
                    <ul><li>${item.SchoolName}, ${item.City}</li>
                    <li>GPA: ${item.GPA}</li>
